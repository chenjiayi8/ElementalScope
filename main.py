...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def read_inputs(self):
        ...
        subfolders = self.read_subfolders(self._root)
    ...
    def precondition_left(self, left_data, rows, cols):
        ...
        coreX = round(cols * 3 / 2)
        coreY = round(rows * 3 / 2)
        left_out, _, _ = add_small_to_big_matrix_2d_periodically(
            mask, left_data, coreX, coreY
        )
        return left_out
    ...
    def get_boundary(self, img, threshold_value):
        """
        This function finds the boundary of a binary mask of the dark border in the input image.

        Args:
        - img: A 2D or 3D numpy array of the input image.
        - threshold_value: An integer representing the threshold value for the grayscale image.

        Returns:
        - boundary: A list of four integers representing the left, right, top, and bottom indices of the boundary.
        """
        if len(img.shape) == 3:
    ...
    def find_border(self, sums):
        ...
        - value: An integer representing the value of the border.
    ...
    def plot_diff(self):
        ...
        boundary[0] += offset
        boundary[1] -= offset
        self.ax.imshow(self._comparison_image)
        self.ax.axis("off")
        self.ax.set_xlim([boundary[0], boundary[1]])
        self.ax.set_ylim([boundary[2], boundary[3]])
    ...
    def stitch(self):
        ...
        self.hint(f"Saving hdf5 and element data ...")
    ...
    def choose_folder(self):
        ...
        self.hint(f"Working on: {folder_name}")
        self.initialise()
    ...
    def placeholder(self):
        ...
        rr, cc = disk((size[0] // 2, size[1] // 2), radius=50)
        A[rr, cc] = 1

        # Circle in the top-left for B
        rr, cc = disk((size[0] // 4, size[1] // 4), radius=50)
        B[rr, cc] = 1

        # Use imshowpair with the falsecolor method
        output = imshowpair(A, B, method="falsecolor")
...
def test_main():
    ...
    window = MyMainWindow()
    window.showMaximized()
    root = r"/home/vscode/0Mapping/Boyu/low-Fe Al area"
    with patch(
        "PyQt5.QtWidgets.QFileDialog.getExistingDirectory", return_value=root
    ):
...
if __name__ == "__main__":
    # cProfile.run("test_main()", "profile_output.pstat")
    test_main()
